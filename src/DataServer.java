        ...
        while (true) {
            try {
                ...
                if (req.code == RequestModel.LOAD_PRODUCT_REQUEST) {
                    ...
                    UserModel currentUser = gson.fromJson(req.body, UserModel.class);

                    UserModel user = dao.loadUser(currentUser);
                    ...
                    Integer orderRows = dao.loadOrders();
                    if (orderRows != null) {
                        res.code = ResponseModel.OK;
                        res.body = String.valueOf(orderRows);
                    } else {
                        res.code = ResponseModel.DATA_NOT_FOUND;
                        res.body = "";
                    }
                } else if (req.code == RequestModel.SAVE_PRODUCT_REQUEST) {
                    ProductModel productModel = gson.fromJson(req.body, ProductModel.class);
                    dao.saveProduct(productModel);
                } else if (req.code == RequestModel.SAVE_ORDER_REQUEST) {
                    OrderModel orderMade = gson.fromJson(req.body, OrderModel.class);
                    dao.saveOrder(orderMade);
                } else {
